...
import { htmlToMarkdown, markdownToHtml, splitFrontmatter } from './markdown';
...
function handleInlineClick(
  ...
  if (linkEl instanceof HTMLAnchorElement) {
    ...
    // In read-only mode a plain click follows the link (open a local .md in
    // the editor, anything else in the browser) instead of opening the link
    // editor. Ctrl/Cmd+click always follows the link regardless of mode.
    // preventDefault matters in read-only mode: with contenteditable off the
    // browser would otherwise navigate the webview to the href.
...
export function MarkdownEditor({ markdown, documentPath, readOnly, onChange, onOutlineChange, onEditorReady, onOpenLocalFile }: MarkdownEditorProps) {
  const lastInternalMarkdownRef = useRef(markdown);
  // editorProps handlers capture this ref (not the prop) so paste/drop guards
  // see the current read-only state even though useEditor only reads the
  // editorProps config once at creation.
  const readOnlyRef = useRef(readOnly);
  readOnlyRef.current = readOnly;
  // Preserved verbatim so YAML/TOML front-matter survives the rich-text
  // round-trip; it is stripped before the body reaches the editor and
  // re-joined on every export instead of being regenerated by turndown.
  ...
  const editor = useEditor({
    ...
    editorProps: {
      ...
      handleDOMEvents: {
        click(view, event) {
          ...
          return handleInlineClick(view, event, setInlineEdit, onOpenLocalFile, setTableMenu, documentPath, readOnlyRef.current);
        },
    ...
    onUpdate({ editor }) {
      const nextMarkdown = frontmatterRef.current + htmlToMarkdown(editor.getHTML());
  ...
  // Toggle the editor's editable state when the read-only flag flips without
  ...
  useEffect(() => {
    ...
    const incomingHtml = markdownToHtml(body, documentPath);
...
export function SourceMarkdownEditor({ markdown, readOnly, onChange, onOutlineChange, onEditorReady, onSourceReady }: SourceMarkdownEditorProps) {
  ...
  // ── Replace ──
  const handleReplace = useCallback(() => {
    if (readOnly || searchMatches.length === 0) return;
    const match = searchMatches[currentMatchIndex];
    if (!match) return;
    const next = markdown.slice(0, match.from) + replaceText + markdown.slice(match.to);
    onChange(next);
  }, [readOnly, searchMatches, currentMatchIndex, markdown, replaceText, onChange]);
  ...
  const handleReplaceAll = useCallback(() => {
    ...
    onChange(result);
  }, [readOnly, searchMatches, markdown, replaceText, onChange]);
