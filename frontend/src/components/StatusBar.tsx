...
export function StatusBar({ path, isDirty, lastSavedAt, stats }: StatusBarProps) {
  const savedLabel = isDirty ? t('status.unsaved') : lastSavedAt ? t('status.savedAt', { time: formatTime(lastSavedAt) }) : t('status.saved');

  return (
    <footer className="statusbar">
      <div className="statusbar-section path-section" title={path || t('document.untitled')}>
        <FilePenLine size={14} />
        <span>{path || t('document.untitled')}</span>
      </div>
